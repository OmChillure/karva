...
#[test]
fn run_ignored_all_runs_skipped_alongside_normal() {
...
#[test]
fn run_ignored_only_runs_only_skipped_tests() {
    let context = TestContext::with_file("test.py", MIXED_TESTS);
...
#[test]
fn run_ignored_default_value_is_all() {
    let context = TestContext::with_file("test.py", MIXED_TESTS);
    assert_cmd_snapshot!(context.command_no_parallel().arg("--run-ignored"), @"
    success: false
    exit_code: 1
    ----- stdout -----
        Starting 3 tests across 1 worker
...
#[test]
fn run_ignored_all_with_skipif_false_condition() {
...
#[test]
fn run_ignored_only_with_no_skipped_tests() {
    let context = TestContext::with_file(
        "test.py",
        r"
def test_alpha():
